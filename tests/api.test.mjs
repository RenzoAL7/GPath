...
test('health and readiness are available without optional integrations', async (t) => {
...
test('read-only routes, HEAD, retired endpoints and 404', async (t) => {
  const { url } = await fixture(t)
  assert.equal((await fetch(url + '/api/probe', { method: 'POST', body: 'data' })).status, 405)
  assert.equal((await fetch(url + '/unknown')).status, 404)
  assert.equal((await fetch(url + '/api/jobs')).status, 404)
  assert.equal((await fetch(url + '/api/releases')).status, 404)
